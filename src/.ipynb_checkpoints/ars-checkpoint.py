# src/ars.py  (DRAFT -- complete _load_ar() after running inspect_ar_checkpoint.py)
"""
Stage 5 (optional): ARS -- AR-verified salience.
...
# diagnose_ar_suffix.py -- single spaces, no newlines; an earlier version
# of this template had newlines added by mistake, which tokenized
# differently and shifted the critic_suffix_ids alignment):
...
def _load_ar(checkpoint_id):
    ...
    (after its final RMSNorm, i.e. what output_hidden_states=True's last
    entry already gives you) at the suffix-anchored last token position.
    The paper's "learned affine map" appears to have collapsed, in this
    open release, into training the whole truncated model end-to-end so
    that its own last hidden state IS the reconstruction -- no extra
    parameters needed beyond a standard (truncated) Qwen checkpoint.

    This is the best-supported reading given the evidence, but it has not
    been empirically verified yet (see smoke_test_ar.py) -- treat it as
    provisional until a real explanation's reconstruction is checked
    against a real Stage-1 vector for the same item.
    """
...
def _reconstruct(ar_model, tokenizer, explanation_text, critic_suffix_ids, layer_index=20):
    ...
    IMPORTANT: uses hidden_states[layer_index] (the RAW output of decoder
    layer `layer_index`), NOT hidden_states[-1]. Loading this checkpoint
    with AutoModelForCausalLM prints:
        "Some weights ... were not initialized from the model checkpoint
         ... ['lm_head.weight', 'model.norm.weight']"
    meaning the final RMSNorm (model.norm) that HF applies to produce
    hidden_states[-1] is RANDOMLY INITIALIZED, not trained -- using it
    would corrupt the reconstruction with untrained noise. hidden_states
    at layer_index (before any final norm) is the raw decoder output,
    matching exactly what extraction.py reads from the target model
    (Qwen) in Stage 1 -- the AR was trained to reconstruct THAT quantity.

    Safety check: the tokenized prompt must end in EXACTLY
    critic_suffix_ids (from nla_meta.yaml) -- the AR is suffix-anchored,
    with no marker-token scan, so if the prompt doesn't end where the AR
    expects, the reconstruction is meaningless.
    """
    import torch
...
def compute_ars_for_item(item, ar_model, tokenizer, critic_suffix_ids):
    ...
    for pos, gradings in item["gradings"].items():
        ...
        for j, g in enumerate(gradings):
            if not (g.get("mentions_colombia") and g.get("quote_verified")):
                continue
